...
export function mountTeachingInteraction(root,model=interactionModel,{force=false}={}){
 ...
 const flandersCard=document.createElement('section');flandersCard.className='mi-section mi-inline mi-dashboard-card';flandersCard.innerHTML=`<div class="mi-heading"><h3>弗兰德斯互动分析 ${button('data-mi-open="flanders" aria-label="查看弗兰德斯指标定义"','ⓘ')}</h3>${button('data-mi-open="flanders"','数据解读 ›')}</div>${radar()}`;
 ...
 const positive=document.createElement('section');positive.className='mi-dashboard-card mi-positive-card';positive.innerHTML=`<div class="mi-dashboard-head"><h3>互动积极性 <button type="button" class="method-info" title="结合课堂原话与上下文观察学生主动提问、积极回应和互动延续。" aria-label="互动积极性定义">${icon}</button></h3>${button('data-mi-open="initiative"','查看证据 ›')}</div><div class="mi-positive-metrics">${categories.map(([k,n])=>`<div><span>${n}</span><strong>${count(k)}<small>次</small></strong></div>`).join('')}</div>`;
 grid.replaceChildren(flandersCard,combined,positive);
