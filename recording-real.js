...
export function mountRealRecording(root,{toast}) {
  ...
  const summary=document.createElement('section');summary.className='real-classroom-summary';summary.innerHTML='<div class="real-summary-head"><h2>PPT摘要</h2><span>按课堂时间顺序</span></div><div class="real-summary-tabs"><button type="button" class="active" data-summary-filter="all">全部</button><button type="button" data-summary-filter="ppt">PPT</button><button type="button" data-summary-filter="video">视频</button><button type="button" data-summary-filter="desktop">电脑桌面</button></div><div class="real-summary-timeline"></div>';
  panel.innerHTML='';panel.append(summary);
  const summaryNodes=[
    {kind:'ppt',time:1440,title:'人文社科类人才',desc:'介绍智慧医疗中人文社科类人才的职责与价值，教师结合案例进行讲解。',duration:'2分36秒',image:'assets/recording-screen-20260912/slide-1440.jpg'},
    {kind:'video',time:2219,title:'便携式超声机案例视频',desc:'播放便携式超声机案例，说明人工智能辅助诊断的应用场景。',duration:'1分46秒',image:'assets/recording-screen-20260912/screen-2400.jpg'},
    {kind:'desktop',time:2520,title:'切换并打开课堂案例材料',desc:'教师切换桌面窗口并打开案例材料，继续讲解案例内容。',duration:'38秒',image:'assets/recording-screen-20260912/screen-2520.jpg'}
  ];
  const summaryFmt=fmt;
  function renderSummary(filter='all'){
    summary.querySelector('.real-summary-timeline').innerHTML=summaryNodes.filter(n=>filter==='all'||n.kind===filter).map(n=>`<article class="real-summary-node ${n.kind}"><time>${summaryFmt(n.time)}</time><span class="real-summary-dot"></span><img src="${n.image}" alt="${n.title}代表画面"><div class="real-summary-copy"><span class="real-summary-label">${n.kind==='ppt'?'PPT':n.kind==='video'?'视频':'电脑桌面'}</span><h3>${n.title}</h3><p>${n.desc}</p><small>持续 ${n.duration} · 对应时段 ASR 原文</small><button type="button" class="real-summary-jump" data-real-seek="${n.time}">定位原视频</button></div></article>`).join('');
  }
  renderSummary();
  summary.addEventListener('click',e=>{const tab=e.target.closest('[data-summary-filter]');if(!tab)return;summary.querySelectorAll('[data-summary-filter]').forEach(b=>b.classList.toggle('active',b===tab));renderSummary(tab.dataset.summaryFilter);});
